...
func UpdateRancherReferences(ctx context.Context, ghClient *github.Client, tag, rancherRepoName, rancherRepoOwner, cliUpstreamURI, cliReleaseBranch, cliRepoName, githubUsername string, dryRun bool) error {
	commitSHA, err := repository.RefCommitSHA(ctx, ghClient, rancherRepoOwner, rancherRepoName, "tags/"+tag)
	if err != nil {
		return err
	}

	if err := updateRancherReferencesAndPush(tag, cliReleaseBranch, commitSHA, cliUpstreamURI, dryRun); err != nil {
		return err
	}
...
func updateRancherReferencesAndPush(tag, releaseBranch, rancherCommitSHA, cliUpstreamURI string, dryRun bool) error {
